...
- طراحی **کاملاً خودکفا** بدون وابستگی به CDN خارجی (Tailwind حذف شد) و با فونت Vazirmatn محلی.
- فرم ورودی راهنمای‌شده با سناریوهای نمونه، اعتبارسنجی داخلی و Toast برای پیام‌ها.
- **ادامه پرونده قبلی** پس از رفرش مرورگر (ذخیره در `localStorage`).
- احراز هویت با هدر `X-Job-Token` و دانلود فایل از طریق Blob (توکن در URL قرار نمی‌گیرد).
- جداول **فیلترشونده/مرتب‌شونده** (جستجو، فیلتر کشور، مرتب‌سازی با کلیک) برای Longlist، امتیاز و منابع.
- **ماشین‌حساب تعاملی Landed Cost** برای مقایسه سناریوها بدون بازکردن اکسل.
- **مقایسه رو‌در‌روی تأمین‌کنندگان** و نمای **چاپی خلاصه اجرایی**.
- کپی یک‌کلیک ایمیل RFQ؛ صفحه اجرا با زمان‌سنج و خط زمانی مرحله.
